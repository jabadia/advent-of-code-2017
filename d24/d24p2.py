...
def dfs(current, bridges, visited, path_len):
    max_val = path_len, 0
    for node in bridges[current]:
        if node not in visited:
            val = dfs(node.target, bridges, visited.union([node, Bridge(*reversed(node))]), path_len + 1)
            max_val = max(
                max_val,
                (val[0], val[1] + node.source + node.target)
            )
