...
def get_weights(discs, disc_name):
    disc = discs[disc_name]
    if not disc['holding']:
        # print("%s, %d %d" % (disc_name, disc['weight'], 0))
        return disc['weight'], 0

    weights = sorted([get_weights(discs, held) for held in disc['holding']])
    assert len(weights) >= 3 or len(weights) == 1 or weights[0][0] == weights[1][0]
    if weights[0][0] != weights[1][0]:  # el primero es distinto
        print(weights[1][0] - weights[0][1])
        exit()
    elif weights[0][0] != weights[-1][0] : # el ultimo es distinto
        print(weights[0][0] - weights[-1][1])
        exit()

    children_weights = sum(w[0] for w in weights)
    # print("%s, %d %d" % (disc_name, disc['weight'] + children_weights, children_weights))
    return disc['weight'] + children_weights, children_weights
...
def solve(program):
    ...
    discs = {}
    for l in lines:
        groups = re.match(r'([a-z]{3,8}) \((\d+)\)( -> ([a-z, ]+))?', l)
        disc, weight, holding = groups[1], int(groups[2]), groups[4]
        discs.update({
            disc: {
                'weight': weight,
                'holding': [d for d in holding.split(', ')] if holding else None,
            }
        })

    held = {held for name, disc in discs.items() if disc['holding'] for held in disc['holding']}
    root = (set(discs.keys()) - held).pop()
    get_weights(discs, root)
