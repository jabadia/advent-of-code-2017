...
def parse_particle(particle_str):
    p, v, a, _ = particle_str.split('>')
    p = [int(c) for c in p.split('<')[1].split(',')]
    v = [int(c) for c in v.split('<')[1].split(',')]
    a = [int(c) for c in a.split('<')[1].split(',')]
    return Particle(p, v, a)


def manhattan_module(v):
    return abs(v[0]) + abs(v[1]) + abs(v[2])


def do_step(particle):
    v = add_vector(particle.v, particle.a)
    p = add_vector(particle.p, v)
    return Particle(p, v, particle.a)


def check_collisions(particles):
    collisions = sorted(particles, key=attrgetter('p'))
    particles = [p for i, p in enumerate(collisions) if
                 (i == 0 or p.p != collisions[i - 1].p) and (i == len(collisions) - 1 or p.p != collisions[i + 1].p)]
    # particles = [list(g[1]) for g in groupby(collisions, key=attrgetter('p'))]
    # particles = [g[0] for g in particles if len(g) == 1]
    return particles


def solve(input):
    particles = [parse_particle(p) for p in input.strip().split('\n')]

    iterations_without_change = 0
    remaining = len(particles)
    while True:
        particles = check_collisions(particles)
        particles = list(map(do_step, particles))
        if remaining == len(particles):
        ...
        if iterations_without_change > 100:
            break
